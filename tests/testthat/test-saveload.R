...
test_that("Basic save and load works", {
  ...
  srtdisk::SaveH5Seurat(obj, tmp, overwrite = TRUE, verbose = FALSE)

  loaded <- tryCatch(
    srtdisk::LoadH5Seurat(tmp, verbose = FALSE),
    error = function(e) {
      if (grepl("slots", e$message, fixed = TRUE)) {
        skip("LoadH5Seurat V5 slot assembly issue with minimal objects")
      }
      stop(e)
    }
  )
...
test_that("Visium spatial coordinates align with metadata", {
  ...
  converted <- srtdisk:::ConvertH5ADSpatialToSeurat(
    h5ad_file = h5,
    seurat_obj = obj,
    assay_name = "Spatial",
    verbose = FALSE
