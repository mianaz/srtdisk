...
For this vignette, we'll use the `pbmc_small` dataset that comes bundled with the package. This dataset showcases saving and loading a dataset with multiple assays, dimensional reductions, and nearest-neighbor graphs.

```{r load_data}
# Load bundled pbmc_small dataset
brain <- readRDS(system.file("testdata", "pbmc_small.rds", package = "srtdisk"))
brain
```

The data loaded is already processed with basic analyses performed.
...
```{r save_data}
SaveH5Seurat(brain, filename = "pbmc_small.h5Seurat", overwrite = TRUE)
```

This process is quick and results in a compact on-disk file.

```{r file_size}
size <- file.size("pbmc_small.h5Seurat")
print(structure(size, class = "object_size"), units = "Kb")
```
...
```{r hfile}
hfile <- Connect("pbmc_small.h5Seurat")
hfile
```
...
# Check if SCT assay exists before accessing
if ("SCT" %in% names(hfile[["assays"]])) {
  hfile[["assays/SCT"]]
} else {
  # Use the default RNA assay if SCT doesn't exist
  hfile[["assays/RNA"]]
}

hfile[["reductions"]]

# Check if umap reduction exists before accessing
if ("umap" %in% names(hfile[["reductions"]])) {
  hfile[["reductions/umap"]]
} else if ("pca" %in% names(hfile[["reductions"]])) {
  # Use pca if umap doesn't exist
  hfile[["reductions/pca"]]
}
```
...
```{r load_full_data, eval=FALSE}
brain2 <- LoadH5Seurat("pbmc_small.h5Seurat")
brain2
```
...
```{r assay_load}
brain2 <- LoadH5Seurat("pbmc_small.h5Seurat", assays = "SCT")
brain2
```
...
```{r slot_load}
brain2 <- LoadH5Seurat("pbmc_small.h5Seurat", assays = "data")
brain2
```
...
```{r assay_slot_load}
brain2 <- LoadH5Seurat("pbmc_small.h5Seurat", assays = list("SCT" = c("data", "scale.data"), "Spatial" = "counts"))
brain2
```
...
```{r bad_reduc}
brain2 <- LoadH5Seurat("pbmc_small.h5Seurat", assays = "Spatial", reductions = "pca")
brain2
```
...
```{r custom_load}
brain2 <- LoadH5Seurat("pbmc_small.h5Seurat", assays = list("SCT" = c("data", "scale.data"), "Spatial" = "counts"), reductions = NA, graphs = FALSE, images = NULL)
brain2
```
...
```{r load_partial}
brain2 <- LoadH5Seurat("pbmc_small.h5Seurat", assays = c("SCT" = "data"), reductions = FALSE, graphs = FALSE, images = NULL)
brain2
```
...
```{r append_partial}
brain2 <- AppendData("pbmc_small.h5Seurat", brain2, assays = FALSE, reductions = NA, graphs = FALSE, images = NULL)
brain2
```
...
```{r append_parital2}
brain2 <- AppendData("pbmc_small.h5Seurat", brain2, assays = "SCT", reductions = FALSE, graphs = FALSE, images = FALSE)
brain2
```
...
```{r append_full}
brain2 <- AppendData("pbmc_small.h5Seurat", brain2)
brain2
```

## Working with external Visium datasets

While `SeuratData` provides curated examples, many Visium datasets are shared as AnnData (`.h5ad`) files through resources such as [cellxgene](https://cellxgene.cziscience.com). The `srtdisk` package can download these files, convert them to h5Seurat, and reuse the partial loading patterns demonstrated above.

### Downloading the Heart Visium dataset

The CZ Biohub Spatial Atlas heart dataset is distributed as an AnnData (`.h5ad`) archive that contains multiple Visium libraries. The command below downloads the file to the working directory; the chunk remains disabled during vignette builds to avoid large network transfers.

```{r heart_visium_download, eval = FALSE}
download.file(
  url = "https://datasets.cellxgene.cziscience.com/8cc521c8-c4ff-4cba-a07b-cae67a9dcba9.h5ad",
  destfile = "sn.heart.h5ad",
  mode = "wb"
)
```

### Converting and isolating a single library

After downloading, convert the AnnData file to h5Seurat, load it, and pull out one Visium library. The metadata column that labels libraries is typically named `library_id`; adjust `library_column` below if your download uses a different field.

```{r heart_visium_convert, eval = FALSE}
Convert("sn.heart.h5ad", dest = "sn.heart.h5seurat", overwrite = TRUE, verbose = FALSE)

heart <- LoadH5Seurat("sn.heart.h5seurat")

library_column <- "library_id"
metadata <- heart[[]]
stopifnot(library_column %in% names(metadata))

library_ids <- unique(metadata[[library_column]])
selected_id <- library_ids[[1]]

selected_cells <- rownames(metadata)[metadata[[library_column]] == selected_id]

heart_library <- subset(heart, cells = selected_cells)

SaveH5Seurat(
  heart_library,
  filename = sprintf("sn.heart.%s.h5seurat", selected_id),
  overwrite = TRUE,
  verbose = FALSE
)
...
You can now explore and append additional data just as we did with the brain example, taking advantage of on-disk storage to manage large spatial datasets.
