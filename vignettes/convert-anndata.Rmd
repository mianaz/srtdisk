...
# Try to use scverse conda environment if available, otherwise use default Python
if (reticulate::conda_binary() != "" && "scverse" %in% reticulate::conda_list()$name) {
...
To showcase going from a `Seurat` object to an AnnData file, we'll use the `pbmc_small` dataset that comes bundled with the package. This is a small subset of a PBMC dataset that's useful for demonstration purposes.

```{r get_data, message=FALSE, warning=FALSE}
# Load bundled pbmc_small dataset
pbmc_small <- readRDS(system.file("testdata", "pbmc_small.rds", package = "srtdisk"))
pbmc_small
```

This is a small preprocessed Seurat object with basic analyses already performed.

The dataset is annotated as below:
```{r}
DimPlot(pbmc_small, reduction = "tsne", label = TRUE, pt.size = 0.5) + NoLegend()
```
...
```{r convert_seurat}
SaveH5Seurat(pbmc_small, filename = "pbmc_small.h5Seurat")
Convert("pbmc_small.h5Seurat", dest = "h5ad")
```

We can view the AnnData file in Scanpy by using the `read_h5ad` function

```{python load_adata, eval = FALSE}
# Note: This code requires scanpy to be installed
# pip install scanpy
import scanpy
adata = scanpy.read_h5ad("pbmc_small.h5ad")
adata
```

And visualize the UMAP with cluster annotations:
```{python plot_adata, fig.width=10, fig.height=8, eval = FALSE}
# Note: This code requires scanpy and matplotlib to be installed
import scanpy as sc
...
To showcase going from an AnnData file to a `Seurat` object, we'll use a processed version of the PBMC 3k dataset that comes bundled with the package. This dataset was processed using Scanpy following [Scanpy's PBMC 3k tutorial](https://scanpy-tutorials.readthedocs.io/en/latest/pbmc3k.html).

```{r get_adata}
# Copy bundled pbmc3k_final.h5ad to working directory
file.copy(
  system.file("testdata", "pbmc3k_final.h5ad", package = "srtdisk"),
  "pbmc3k_final.h5ad",
  overwrite = TRUE
)
```
...
```{r plot_heart_multi, fig.width=10, fig.height=8}
