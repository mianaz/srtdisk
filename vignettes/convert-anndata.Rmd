...
Historically, converting an AnnData file to a `Seurat` object was a two-step process: convert the AnnData file to h5Seurat with [`Convert`](https://mojaveazure.github.io/seurat-disk/reference/Convert.html), then [load the h5Seurat file](https://mojaveazure.github.io/seurat-disk/reference/LoadH5Seurat.html). That workflow continues to be supported and remains a convenient way to materialise an on-disk h5Seurat representation for repeated use.
...
SeuratDisk now supports loading AnnData files directly into `Seurat` objects without producing an intermediate h5Seurat file. This is the recommended approach when you simply want to bring an AnnData analysis into Seurat.

```{r load_direct}
pbmc3k_direct <- LoadH5AD("pbmc3k_final.h5ad")
pbmc3k_direct
```

Both workflows yield equivalent in-memory objects; choose the path that best matches your downstream needs.

### Spatial datasets

When an AnnData file contains 10x Genomics Visium information, `LoadH5AD` reconstructs the spatial images, scalefactors, and coordinates so that Seurat spatial tooling works out of the box.
