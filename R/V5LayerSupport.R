...
WriteV5Layer <- function(h5_group, layer_name, matrix_data, features = NULL, verbose = FALSE) {
  vmessage(paste0("Writing V5 layer '", layer_name, "'"), verbose)
  ...
  if (layers_group$exists(layer_name)) {
    vmessage(paste0("Removing existing layer '", layer_name, "'"), verbose)
  ...
  if (is_sparse) {
    ...
    vmessage(paste0("  Wrote sparse matrix: ", length(matrix_data@x), " non-zero values, ",
            "shape ", nrow(matrix_data), " x ", ncol(matrix_data)), verbose)
  } else {
    # Write dense matrix directly
    layers_group[[layer_name]] <- as.matrix(matrix_data)

    vmessage(paste0("  Wrote dense matrix: shape ", nrow(matrix_data), " x ", ncol(matrix_data)), verbose)
  }
  ...
  if (!is.null(features) && !h5_group[["meta.data"]]$exists("_index")) {
    ...
    vmessage(paste0("  Stored ", length(features), " feature names"), verbose)
  }
...
ReadV5Layer <- function(h5_group, layer_name, features = NULL, cells = NULL, verbose = FALSE) {
  ...
  if (!h5_group$exists("layers")) {
    vmessage("No layers group found", verbose)
    return(NULL)
  }
  ...
  if (!layers_group$exists(layer_name)) {
    vmessage(paste0("Layer '", layer_name, "' not found"), verbose)
    return(NULL)
  }
  ...
  if (inherits(layer_obj, "H5Group")) {
    ...
    vmessage(paste0("Reading sparse V5 layer '", layer_name, "'"), verbose)
    ...
    # Dense matrix format
    vmessage(paste0("Reading dense V5 layer '", layer_name, "'"), verbose)
  ...
  if (!is.null(features) && !is.null(cells)) {
    ...
    if (nrow(mat) != expected_nrows || ncol(mat) != expected_ncols) {
      ...
      if (nrow(mat) == expected_ncols && ncol(mat) == expected_nrows) {
        vmessage("Transposing matrix to match expected dimensions", verbose)
        mat <- t(mat)
      } else if (nrow(mat) < expected_nrows && ncol(mat) == expected_ncols) {
        # Expand to full feature space
        vmessage("Expanding matrix to full feature space", verbose)
