...
LoadH5MU <- function(file,
  ...
  # Open h5mu file to inspect structure and read global obs
  # Note: MuDataSeurat::ReadH5MU may invalidate this handle, so we re-open later if needed
  ...
  tryCatch({
    seurat_obj <- MuDataSeurat::ReadH5MU(file)
  }, error = function(e) {
    stop(
  ...
  for (old_name in names(assay.names)) {
    if (old_name %in% current_assays && assay.names[old_name] != old_name) {
      ...
      if (old_name == DefaultAssay(seurat_obj)) {
        DefaultAssay(seurat_obj) <- new_name
      }
  ...
  # Re-open h5mu file since MuDataSeurat::ReadH5MU may have closed the original handle
  if (restore.spatial) {
    h5mu_reopen <- tryCatch(H5File$new(file, mode = "r"), error = function(e) NULL)
    if (!is.null(h5mu_reopen)) {
      on.exit(h5mu_reopen$close_all(), add = TRUE)
      seurat_obj <- RestoreSpatialFromH5MU(
        h5mu_file = h5mu_reopen,
...
ConvertH5MUSpatialToSeurat <- function(h5mu_file, seurat_obj, modality,
  ...
  # Add coordinates to metadata and store Visium info in misc
